import pygame
import random
# Y 2018/11/17 imported the time library
import time
...
class Player:
    ...
    def __init__(self, gameObj, colour, playerid, x, y, initialDirection):
        ...
        # Y 2018/11/17 declaring variable boost and setting it to 0
        self.boost = 0;  # its value is 1 if the player is boosting
        # Y 2018/11/17 declaring variable b_time and setting it to 3. This is used for boosting time.
        self.b_time = 3;
        # Y 2018/11/17 declaring start_boost and setting it to time.time(). This is used for handling the boost length.
    ...
    #Y/2018/12/10/added codes for handling boost
    def movePlayer(self):
        if time.time() - self.start_boost >= 0.5:  # limits boost to 0.5s
            self.boost = 0
        # move the player (change position) and update the game board
        if self.direction == Player.DIRECT_UP:
            if self.boost== 1:
                self.game.board.board[self.posY - 2][self.posX] = self.playerid
            ...
            if self.boost == 1:
                self.game.board.board[self.posY][self.posX + 2] = self.playerid
    ...
    # Y 2018/12/10 added if statement to handle case of boost
    def wouldCollideSelf(self, nextDirection):
        ...
        if self.boost == 1:
            return (nextDirection == Player.DIRECT_UP and self.posY - 2 == self.prevPos['y']) or \
                   (nextDirection == Player.DIRECT_RIGHT and self.posX + 2 == self.prevPos['x']) or \
                   (nextDirection == Player.DIRECT_DOWN and self.posY + 2 == self.prevPos['y']) or \
                   (nextDirection == Player.DIRECT_LEFT and self.posX - 2 == self.prevPos['x'])
    ...
    def wouldCollide(self, direction):
        ...
        if self.boost == 1:
            ...
            return (direction == Player.DIRECT_UP and self.game.board.isObstacle(self.posX, self.posY - 2)) or \
                   (direction == Player.DIRECT_RIGHT and self.game.board.isObstacle(self.posX + 2, self.posY)) or \
                   (direction == Player.DIRECT_DOWN and self.game.board.isObstacle(self.posX, self.posY + 2)) or \
                   (direction == Player.DIRECT_LEFT and self.game.board.isObstacle(self.posX - 2, self.posY))or \
                   (direction == Player.DIRECT_UP and self.game.board.isObstacle(self.posX, self.posY - 1)) or \
    ...
    def directionToNextLocation(self, posX, posY, direction):
        ...
        if self.boost == 1:
            if direction == Player.DIRECT_UP: return (posX, posY - 2)
            if direction == Player.DIRECT_RIGHT: return (posX + 2, posY)
            if direction == Player.DIRECT_DOWN: return (posX, posY + 2)
            if direction == Player.DIRECT_LEFT: return (posX - 2, posY)
        else:
...
class Human(Player):
    def __init__(self, gameObj, colour, playerid, x, y, initialDirection, controls):
        ...
        # controls is a 5-tuple, up, right, down, left,boost
        self.ctl_up = controls[0]
        self.ctl_right = controls[1]
        self.ctl_down = controls[2]
        self.ctl_left = controls[3]
        self.ctl_boost = controls[4]
    #y/12/10/calling boost method
    def event(self, event):
